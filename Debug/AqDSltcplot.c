...
//色関係	
	int dBmax=10;		//表示の最大dBを規定　結構大事だけど目だってない
	...
	sprintf(ope,"old_Qlhead_%d%02d%02d.txt",yy,mm,dd);
	sprintf(fnm,"%d%02d%02d_QLcheck.txt",yy,mm,dd);
//	printf("%s\n",ope);
	ptr = fopen(ope, "r");
	if (ptr == NULL){
...
//					data[m] = data[m] - ave[m];
//					ci=zerocol+data[m]*12;	//色を決定
					ci=zerocol+data[m]*(cimax/dBmax);	//0～dBmax dBまでをプロットする
					if(ci<zerocol){
						ci=0;
					}
					else if(ci>zerocol+cimax-1){
						ci=zerocol+cimax-1;
					}
					cpgsci(ci);	//カラーとz軸の対応
...
//					ci=zerocol+data[m]*12;	//色を決定
		...
		cpgsch(1.35);
		sprintf(fnm2,"Test: NOT Supported %d/%02d/%02d",yy,mm,dd);
		cpgtext(53, 96, fnm2);		
		fclose(ptr);
		cpgclos();
		sprintf(fnm2,"IPRT_SUN_%02d%02d%02d.gif",yy,mm,dd);
