...
改修内容
・8bitデータの飽和有無を確認する(白：飽和有・赤>15dB、緑：10-15dB、青：<10dB・黒：BG以下)
...
//色関係	
	...
	if (argc == 1){
	  ...
	  printf("date:%04d/%02d/%02d\n",yy,mm,dd);
		sprintf(ope,"%04d%02d%02d/Qlhead_%04d%02d%02d.txt",yy,mm,dd,yy,mm,dd);
	}
...
//平均ファイル読み込み
	...
	if (ptr2 == NULL){
		printf("File Open Error: qsun_bg_low.out\n");
		return 0;
	}
	for(m=0;m<1024;m++){
		fscanf(ptr2,"%f",&ave[m]);
...
//		cpgwedg("RI", 1.0f, 3.0f, 0, dBmax, "dB from BG");
		cpgsch(0.75);
		sprintf(label,"<0dB:Black 0-10dB:Blue");
		cpgmtxt("R",1.5,0.5,0.5,label);
		sprintf(label,"10-15dB:Green 15-25dB:Red >25dB:White");
		cpgmtxt("R",2.5,0.5,0.5,label);
		cpgsch(1.0);
