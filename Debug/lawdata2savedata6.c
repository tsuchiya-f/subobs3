...
//平均ファイル読み込み
	...
	if (ptr2 == NULL){
		printf("   File Open Error qsun_bg_high.out\n");
		return 0;
	}
	for(m=0;m<bin;m++){
		fscanf(ptr2,"%f",&ave[m]);
...
//解析する日、解析bitを決めるファイルを読み込む
	...
	if (fp2 == NULL){
		printf("   File Open Error; lawdata2arc.txt\n");
		return 0;
	}
...
//				sprintf(fnm,"20101002_%d.txt",pic*fileMax + file);
				...
				ptr = fopen(fnm, "rb");
				if (ptr == NULL){
					printf("\n   File Open Error : %s",fnm);
					memset(outr,0x00,sizeof(outr));
