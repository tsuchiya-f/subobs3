...
変更履歴：
2013/10/07　(Tsuchiya) 事後アーカイブ処理のために、岩井作lawdata2savedata6.cを改修
改修内容
・ファイルパスの更新
*/
...
int main(){
	...
	char dir[200],path[200],send[200],copy[200];	//ディレクトリ操作する時に使用
...
//以下はディレクトリ操作の部分。できたディレクトリを、その中にあるファイルごとサーバーにコピー
		sprintf(path,"%d%02d%02d",yy,mm,dd);	//直下に日付に対応したディレクトリ
		_mkdir(path);			//新しいディレクトリを作る	

		sprintf(fnm3,"Qlhead_%d%02d%02d.txt",yy, mm, dd);//start stop dataをコピーする
		sprintf(send,"../Debug/%s/%s",path,fnm3);	//元ディレクトリにあるコピーしたいファイル名
		sprintf(copy,"%s/%s",path,fnm3);	//コピーした後のフルパス
		CopyFile(send,copy,TRUE);//FALSEだとファイルの上書き

		sprintf(fnm3,"../Debug/%04d%02d%02d/%d%02d%02d_head.txt",yy, mm, dd, yy, mm, dd); //time data を開く
		printf("%s\n",fnm3);
		ptr3 = fopen(fnm3, "rb");
		if (ptr3 == NULL){
...
//			sprintf(fnm2,"20101002_Lv1_%d.txt",pic);
			sprintf(fnm2,"%s\\%d%02d%02d_Lv1_%d-%d.dat",path,yy,mm,dd,pic*fileMax,pic*fileMax+4);
			fp=fopen(fnm2,"wb+");
			printf("OK_8_2( ");
//ここからR固有のルーティン
			for(file=0;file<fileMax;file++){	
			//処理データファイル
				sprintf(fnm,"../Debug/%d%02d%02d_%d.txt",yy,mm,dd,pic*fileMax + file);
